       ...
       begin
           case (state)
	        ...
	        COMP :		// complete the code for the Data path and State transition operations
	          begin 
	             // RTL operations in the Data Path   		                  
					if(M[I] < Min) Min <= M[I];                // if M[I] is less than Min
										// then load Min with M[I]
				    if(M[I] > Max) Max <= M[I];                // if M[I] is greater than Max
										// then load Max with M[I]
					I <= I+1; 			// Increment I unconditionally
					// state transitions in the control unit       
					if (I == 15) state <= DONE;               // If I reaches its TC (Terminal Count) 
